...
dfa* minimize(dfa* A){
    ...
    int *th = malloc(sizeof(int)*((A->m)+1));
        ...
        #pragma omp parallel num_threads(THREADS)
        {
            ...
            #pragma omp sections
            {
                ...
                #pragma omp section
                {
                    ...
                    chooseThreshold(count[A->m], th, A->m, k, A->n);
                }

                #pragma omp section
                {
                    doSortHalf1(sort[(A->m)], sort[(A->m)-1], signature[(A->m)-1], count[(A->m)-1], th[(A->m)-1], A->n);
                }

                #pragma omp section
                {
                    doSortHalf2(sort[(A->m)], sort[(A->m)-1], signature[(A->m)-1], count[(A->m)-1], th[(A->m)-1], A->n);
                }
            ...
            #pragma omp sections
            {
                #pragma omp section
                {
                    doSortHalf1(sort[(A->m)+1], sort[(A->m)], signature[(A->m)], count[(A->m)], th[(A->m)], A->n);
                }

                #pragma omp section
                {
                    doSortHalf2(sort[(A->m)+1], sort[(A->m)], signature[(A->m)], count[(A->m)], th[(A->m)], A->n);
                    posArray = sort[(A->m)+1];
                }
